...
http_request_min_upload_ms=50      # حداقل فاصله بین درخواست‌های آپلود
...
http_request_body_param="data"     # نام query parameter برای payloadهای http-request-body
http_request_body_method="GET"     # حالت GET query یا POST body برای http-request-body
auto_update=true
...
resolve_ip=""              # پیش‌رزولو دامنه به IP؛ یک IP، لیست جداشده با کاما، یا آرایه TOML
resolve_ip_mode="least_ping"   # حالت انتخاب IP هنگام چند IP: "least_ping" (پیش‌فرض) یا "round_robin"
resolve_ip_latency_interval=60 # فاصله بین اندازه‌گیری تأخیر مجدد در حالت least_ping (ثانیه)
